...
# This must be the first thing you do to ensure all subsequent
# environment variable lookups are correct.
load_dotenv()

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
app = Flask(__name__)

# --- Configuration for Local vs. AWS Services ---
# Check if the USE_LOCAL_SERVICES environment variable is set to 'true'
use_local_services = os.environ.get("USE_LOCAL_SERVICES", "False").lower() == "true"
aws_region = os.environ.get("AWS_REGION", "us-east-1")

if use_local_services:
    # Use MinIO for S3 and DynamoDB Local for DynamoDB
    print("Using local services (MinIO and DynamoDB Local).")
    ...
    # For local secrets, we'll just read from the environment
    secrets_manager = None
else:
    # Use standard AWS services
...
# For local development, we get the secret directly from the environment
# For AWS, this is the ARN to the secret in Secrets Manager
...
def get_jwt_secret():
    """
    Retrieves the JWT secret. If using local services, it reads
    it directly from the environment. Otherwise, it uses AWS Secrets Manager.
    """
    if use_local_services:
        return JWT_SECRET_ARN # For local, the ARN variable holds the actual secret
    try:
...
def get_file():
    ...
    try:
        ...
        if e.response['Error']['Code'] == '404' or e.response['Error']['Code'] == 'NoSuchKey':
            ...
            app.logger.error(f"S3 file retrieval failed for path {path}: {e}")
            abort(500, "Failed to retrieve file.")
    except Exception as e:
        app.logger.critical(f"Unexpected error retrieving file {path}: {e}")
        abort(500, "Internal server error.")

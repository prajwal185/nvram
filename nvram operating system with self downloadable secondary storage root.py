...
class CloudOSSetup:
    ...
    def create_setup_directory(self):
        ...
        if self.system == 'windows':
            try:
                ...
                logger.warning("⚠️ Could not set secure permissions on Windows")
        else:
    ...
    def install_system_dependencies(self):
        ...
        try:
            ...
            logger.warning(f"⚠️ Some system dependencies may not have installed correctly: {e}")
    def _install_windows_dependencies(self):
        ...
        if not shutil.which('choco'):
            ...
            try:
                ...
                logger.warning("⚠️ Could not install Chocolatey automatically")
        dependencies = [
            'git',
            'curl',
            'wget',
        ...
        if self.system == 'linux':
            ...
            if fuse_conf.exists():
                try:
                    ...
                    logger.warning("⚠️ Cannot modify /etc/fuse.conf - run as root or manually enable user_allow_other")
        elif self.system == 'windows':
    ...
    def setup_gcp_integration(self):
        logger.info("☁️ Setting up Google Cloud Platform integration...")
        gcp_config = {
    ...
    def setup_gcp_credentials(self):
        ...
        if not gcp_key_path.exists():
            ...
            logger.warning("⚠️ Placeholder GCP credentials created. Replace with actual credentials.")
        else:
    ...
    def verify_installation(self):
        ...
        for module in test_imports:
            try:
                ...
                if module == 'fuse':
                    logger.warning("⚠️ FUSE module test failed. This is expected if fuse is not installed or configured correctly.")
                elif module == 'pywin32' or module == 'winfspy':
                    logger.warning(f"⚠️ {module} import test failed. This is expected if the package failed to install.")
                else:
...
def run_client(config):
    logger.info("🖥️ Starting Cloud OS Client...")
    ...
    if mode == 'client':
        ...
        if current_system in ['linux', 'darwin']:
            try:
                ...
                logger.info("🗂️ FUSE support detected")
            except ImportError:
                logger.warning("⚠️ FUSE library (fusepy) not found. Client will run in basic mode.")
        elif current_system == 'windows':
            try:
                from winfspy import FileSystemOperations as WinFspOperations
                logger.info("🗂️ WinFsp support detected")
            except ImportError:
                logger.warning("⚠️ WinFsp library not found. Client will run in basic mode.")
